...
export const setWalletList = (walletList = generateInitialWalletList()) =>
  putFile(dashbord.WALLET_LIST, wlTMPL.walletListTMPL(walletList), PUT_OPTIONS)
    .then(() => walletList);
...
export const logEvent = event =>
  getFile(dashbord.DAPPY_LOG, GET_OPTIONS)
    .then(log => {
      ...
      putFile(dashbord.DAPPY_LOG, JSON.stringify([event, ...logList]), PUT_OPTIONS);
    });

export const getWalletList = () =>
  getFile(dashbord.WALLET_LIST, GET_OPTIONS)
    .then((json) => json ? jsonParse(json) : null);
