import { filter, find, gt, lensPath, map, view, reduce, append, split } from 'ramda';
...
const calculateSentValue = (vout, address, fee) =>
  reduce((acc, el) => (address !== view(lensVout, el) ? +el.value + acc : acc), fee, vout).toFixed(
    8,
  );

const valueFindOut = (vout, address) => find(el => address === view(lensVout, el))(vout).value;

const focusTxHref = currencyName => view(lensPath(['hrefs', currencyName]), config);

const normalizeFuncForInsightApi = (record, address, wtype) => {
  const hrefs = focusTxHref(wtype);
  const type = filter(vin => vin.addr === address, record.vin).length !== 0 ? 'sent' : 'received';
  return {
    type,
    ...
    info: {
      ...
      from: [{ href: `${hrefs.address}${record.vin[0].addr}`, address: record.vin[0].addr }],
      to: [
        {
...
export const normalizeFunctions = {
  ...
  [curNames.DOGE]: (record, address) => {
    ...
    const type =
      filter(inputs => inputs.addresses[0] === address, record.inputs).length !== 0
        ? 'sent'
        : 'received';
    return {
      type,
  ...
  [curNames.BCH]: (record, address) => {
    ...
    const type =
      filter(vin => vin.addr === addrInBCHFormat, record.vin).length !== 0 ? 'sent' : 'received';
    return {
      type,
      time: tsToReadableDate(record.time),
      value:
