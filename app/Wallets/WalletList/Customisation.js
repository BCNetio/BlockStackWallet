...
class Customisation extends React.Component {
  ...
  constructor(props) {
    super(props);
    const gaiaWallet = head(props.wallets.filter(wallet => wallet.wid === props.options.wid));
    this.state = {
      wallet: gaiaWallet,
  ...
  generateRandomPair = () =>
    config.avCurrencyes.get(this.state.wallet.type).name +' '+ take(2, generateMnemonic().split(' ')).join('-');
  ...
  saveUpdates = () => {
    const newWallets = this.props.wallets.map(wallet =>
      wallet.wid === this.props.options.wid
        ? { ...this.state.wallet, alias: this.state.alias ? this.state.alias : this.generateRandomPair() }
        : wallet);
