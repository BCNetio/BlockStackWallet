...
class CreateNewWallet extends React.Component {
  ...
  handleTokenChange = (target, value) => {
    this.setState({
      ...this.state,
      token: { ...this.state.token, [target]: value }
    });
  };
  ...
  saveWallet = () => {
    ...
    this.props.createWallet(
      this.focus ? this.focus : this.props.wallets,
      wallet
    );
    setTimeout(
      () =>
  ...
  isWalletExists = pk =>
    this.props.wallets.find(
      wallet =>
        (wallet.privateKey === pk || wallet.address === pk) &&
        wallet.type === this.state.currency
    );
  ...
  recoverWallet = () => {
    if (/^[13n][1-9A-Za-z][^OIl]{20,40}/.test(this.state.hashKey)) {
      ...
      try {
        this.savePreview(
          recoverWallet(this.state.currency.sysName, this.state.hashKey)
        );
      } catch (e) {
      ...
      <PopupLayout>
        <Popup create>
          ...
          {value === CREATE && (
            <TabContainer>
              <p className="description">
                Please select from the list below the cryptocurrency you want to
                create the new wallet. New wallet will appear in the list of
                your wallets instantly.
              </p>
          ...
          {value === IMPORT && (
            <TabContainer>
              <p className="description">
                Please select the currency name you want to import the existing
              ...
              <br />
              <WalletTypeSelect
                handleChange={this.changeWalletType}
          ...
          {value === TOKENS && (
            <TabContainer>
              ...
              <InputGrey
                ...
                onChange={e =>
                  this.handleTokenChange("address", e.currentTarget.value)
                }
              />
              <InputGrey
                ...
                placeholder="Name"
                onChange={e =>
                  this.handleTokenChange("name", e.currentTarget.value)
                }
              />
              <InputGrey
                ...
                placeholder="Symbol"
                onChange={e =>
                  this.handleTokenChange("symbol", e.currentTarget.value)
                }
              />
              <InputGrey
                ...
                onChange={e =>
                  this.handleTokenChange("decimals", e.currentTarget.value)
                }
              />
              <PopupButton onClick={this.props.closeModal}>
                Add token to list
              </PopupButton>
            </TabContainer>
          )}
