...
export const TokenWrapper = styled.div `
  background: linear-gradient(180deg, rgba(31, 36, 49, 0.4) 0%, rgba(43, 54, 73, 0.4) 100%) no-repeat;
...
export const TokensLayout = withStyles(styles)(({ classes, tokens, callModal, wallet }) => (
  <Card className={classes.card}>
    <TokenWrapper>
      <div className="title-wrapper">
        <p>Tokens</p>
        <button onClick={() => callModal(AddCustomEthToken, { wallet })}>Add tokens</button>
      </div>
      <ul>
        {
          tokens && tokens.map(token =>
            <li key={token.tokenInfo.address}>
              <span>
                {token.balance / (10 ** token.tokenInfo.decimals)} { token.tokenInfo.symbol }
              </span>
              {
                token.tokenInfo.price.rate
                  ? <span>
                      {(token.balance / (10 ** token.tokenInfo.decimals) * parseFloat(token.tokenInfo.price.rate)).toFixed(2)} USD
                  </span>
                  : null
              }
            </li>)
