...
import { Popup, PopupLayout, CloseUpButton, PopupButton, InputGrey } from '../../Views';
  ...
  update = () => {
    ...
    const tokens = options.wallet.tokens
      ? { ...options.wallet.tokens,
          tokenList: [...options.wallet.tokens.tokenList, {balance: 0, tokenInfo: this.state}]
        }
      : { updated: new Date(), tokenList: [{balance: 0, tokenInfo: this.state}]};
      this.props.update(
    ...
    return(
      <PopupLayout>
        <Popup>
          ...
          <p className="title">Add custom token to {this.props.options.wallet.alias}</p>
          <InputGrey
            type="text"
...
export default connect(mapStateToProps, mapDispatchToProps)(AddCustomEthToken);
